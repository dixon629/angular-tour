        ...
        angular.module('angular-tour.tour', []).constant('tourConfig', {
            ...
            function($scope, orderedList) {
                ...
                self.select = function(nextIndex) {
                    ...
                    if (nextIndex > steps.getLastIndex()) {
                ...
                self.nextStep = function() {
                    return self.steps.getNextIndex(self.currentStep);
                };
                self.previousStep = function() {
                    return self.steps.getPreviousIndex(self.currentStep);
                };
            }
        ]).directive('tour', [
            '$parse',
            ...
            'tourConfig',
            'debounce',
            function($window, $compile, $interpolate, $timeout, scrollTo, tourConfig, debounce, orderedList) {
            ...
            OrderedList.prototype.getPreviousIndex = function(current) {
                var index = this._array.indexOf(current);
                var previous;
                if (index > 0) {
                    previous = this._array[index - 1];
                } else {
                    previous = 0;
                }
                return previous;
            ...
            OrderedList.prototype.getNextIndex = function(current) {
                var index = this._array.indexOf(current);
                var next;
                if (index < this._array.length - 1) {
                    next = this._array[index + 1];
                } else {
                    next = current + 1;
                }
                return next;
            ...
            OrderedList.prototype.getLastIndex = function() {
